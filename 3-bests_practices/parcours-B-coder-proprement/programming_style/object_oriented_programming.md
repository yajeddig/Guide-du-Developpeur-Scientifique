...
#### Exemple simple : la classe `Car`

Imaginons que nous voulions modéliser une voiture dans notre programme. Nous pouvons créer une classe `Car` :
...
class Car:
    def __init__(self, brand, model, year):
        ...
        self.model = model
        self.year = year
        self.mileage = 0  # Initialisé à 0 par défaut

    def drive(self, distance):
        self.mileage += distance
        print(f"La {self.brand} {self.model} a parcouru {distance} km.")

    def display_info(self):
        print(f"Voiture : {self.year} {self.brand} {self.model}, Kilométrage : {self.mileage} km")
```

Dans cet exemple :

1. La **méthode `__init__`** est le constructeur de la classe. Elle est appelée automatiquement lorsque vous créez un nouvel objet `Car`. Elle initialise les attributs de l'objet avec les valeurs fournies.
1. Les **méthodes** `drive` et `display_info` définissent les actions que peut effectuer un objet `Car`.
1. Le paramètre `self` fait référence à l'instance actuelle de la classe. Il permet d'accéder aux attributs et méthodes de l'objet.
...
1. En utilisant la méthode "accélérer", vous augmenterez la vitesse de la voiture 1 de 30 km/h et celle de la voiture 2 de 20 km/h.
...
1. Enfin, vous afficherez la vitesse actuelle de chaque voiture en utilisant la méthode `get_speed`.
